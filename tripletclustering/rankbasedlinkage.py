...
def knn_rank_graph(X, n_neighbors, metric, random_state=None):
    query_n_neighbors = min(5, n_neighbors // 5)
    index = pynndescent.NNDescent(
        X, n_neighbors=query_n_neighbors, metric=metric, random_state=random_state
    )
...
class RankBasedLinkage(ClusterMixin, BaseEstimator):
    ...
    random_state: int, RandomState instance or None, optional (default: None)
        ...
        by `np.random`. Only used in KNN rank graph generation.
    ...
    def fit(self, X, y=None):
        ...
        if self.metric == "precomputed":
            ...
            X = check_array(X)
            self.ood_ = knn_rank_graph(
                X, self.n_neighbors, self.metric, self.random_state
            )
        ...
        with ProgressBar(
            ...
            L = mutual_friend_list(
                self.ood_.indptr, self.ood_.indices, UniTuple(int32, 2), progress_bar
            )
        ...
        with ProgressBar(
            ...
            row_ind, col_ind, data = in_sway(
                ood.indptr, ood.indices, ood.data, L, progress_bar
            )
        self.linkage_ = sp.coo_array(
            (data, (row_ind, col_ind)), shape=(X.shape[0], X.shape[0])
        )
        ...
        if self.cluster_selection_method == "threshold":
            ...
            if self.threshold_ is None:
                self.threshold_ = 0
                while (
                    np.sum(self.linkage_.data >= self.threshold_)
                    >= self.linkage_.shape[0]
                ):
                    self.threshold_ += 1
            ...
            self.labels_, self.condensed_tree_ = hslc(
                self.linkage_, self.cluster_selection_method, self.min_cluster_size
            )
        else:
            raise ValueError(
                f"cluster_selection_method must be one of 'threshold', 'eom', or 'leaf'. Got {self.cluster_selection_method}"
            )

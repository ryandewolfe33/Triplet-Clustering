...
def build_mst(similarity: sp.sparray | np.ndarray) -> np.ndarray:
    similarity = check_array(similarity, accept_sparse=True, ensure_min_samples=0, ensure_min_features=0)
...
class TripletClustering(ClusterMixin, BaseEstimator):
    ...
    def _validate_params(self):
        ...
        if self.similarity_method not in available_similarity_methods:
            raise ValueError(f"similarity_method must be one of: {available_similarity_methods}. ")
        ...
        if self.cluster_selection_method not in available_cluster_selection_methods:
            raise ValueError(f"similarity_method must be one of: {available_cluster_selection_methods}. ")
        ...
        if (self.cluster_selection_method == "flat"
        ...
        if (
            not np.issubdtype(type(self.n_neighbors), int)
    ...
    def fit(self, X, y=None):
        ...
        n = X.shape[0]
        print(f"Running Triplet Clustering with similarity {self.similarity_method}, min cluster size {self.min_cluster_size}, and cluster selection method {self.cluster_selection_method}.") if self.verbose else None
        ...
        if self.similarity_method == "precomputed":
            ...
            if self.similarity_method == "pald":
                ...
                ood = knn_rank_graph(X, n_neighbors=self.n_neighbors, metric=self.metric, random_state=random_state, **knn_kwargs)
        ...
        condensed_tree = condense_tree(linkage_tree, min_cluster_size=self.min_cluster_size)
        print("Selecting Clusters") if self.verbose else None
        cluster_selection_kwargs = self.cluster_selection_kwargs if self.cluster_selection_kwargs is not None else {}
        if self.cluster_selection_method == "leaf":
            from .cluster_selection.eom_leaf import eom_leaf
            selected_clusters = eom_leaf(condensed_tree, n, cluster_selection_method="leaf", **cluster_selection_kwargs)
        elif self.cluster_selection_method == "eom":
            from .cluster_selection.eom_leaf import eom_leaf
            selected_clusters = eom_leaf(condensed_tree, n, cluster_selection_method="eom", **cluster_selection_kwargs)
        elif self.cluster_selection_method == "pl":
            from .cluster_selection.pl import pl
            self.labels_, self.extras = pl(condensed_tree, n, self.min_cluster_size, n, **cluster_selection_kwargs)
            selected_clusters = None
        elif self.cluster_selection_method == "flat":
            from .cluster_selection.flat import flat
            if self.cut_value == "method":
                selected_clusters = flat(condensed_tree, similarity, self.similarity_method)
            else:
                selected_clusters = flat(condensed_tree, similarity, self.cut_value)
        else:
            raise ValueError(f"cluster_selection_method '{self.cluster_selection_method}' not recognised.")

...
def hslc(similarity_graph, cluster_selection_method, min_cluster_size):
    if isinstance(similarity_graph, np.ndarray):
        ...
        raise ValueError("similarity_graph must be either a numpy array or a scipy.sparse array.")
    ...
    clusters, membership_strengths, linkage_tree, condensed_tree, sorted_mst = fast_hdbscan.hdbscan.clusters_from_spanning_tree(mst, cluster_selection_method=cluster_selection_method, min_cluster_size=min_cluster_size)

...
def test_rankbasedlinkage_class(data):
    rbl = RankBasedLinkage(n_neighbors=10, metric="precomputed", cluster_selection_method="threshold")
...
def test_rankbasedlinkage_eom(data):
    rbl = RankBasedLinkage(n_neighbors=10, metric="precomputed", cluster_selection_method="eom", min_cluster_size=2)
    predict = rbl.fit(data)
...
def test_rankbasedlinkage_leaf(data):
    rbl = RankBasedLinkage(n_neighbors=10, metric="precomputed", cluster_selection_method="leaf", min_cluster_size=2)
    predict = rbl.fit(data)

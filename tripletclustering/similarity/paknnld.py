...
def cohesion(ood_indptr, ood_indices, ood_data, progress_bar):
    ...
    for x in prange(len(ood_indptr)-1):
        ...
        for offset in range(len(x_friends)):
            ...
            denominator = len(x_friends) + len(y_friends) - len(np.intersect1d(x_friends, y_friends))
...
def paknnld(ood, verbose=False):
    ...
    with ProgressBar(
        ...
        similarity.data = cohesion(similarity.indptr, similarity.indices, similarity.data, progress_bar)
    return similarity
